...
SETTINGS_DIR = $(SRC_DIR)
MODULE_DIRS = $(LOG_APP_DIR) $(HOUSE_DIR) $(NOTIFICATION_DIR) $(LOCK_DIR) $(JSON_DIR) $(LOGGER_DIR) $(LOGGER_WEB_DIR) $(LOGGER_WEB_GRAPH_DIR) $(HTTP_DIR) $(PORTABLE_DIR) $(SETTINGS_DIR)
...
SRC = $(SRC_DIR)/houseNotif.c $(LOG_APP_DIR)/app_startup.c $(HOUSE_DIR)/poller.c $(NOTIFICATION_DIR)/notification_worker.c $(NOTIFICATION_DIR)/notification_lock.c $(LOG_APP_DIR)/web_controls.c $(HOUSE_DIR)/config.c $(HOUSE_DIR)/houseApi.c $(HTTP_DIR)/httpClient.c $(LOCK_DIR)/instanceLock.c $(JSON_DIR)/jsonUtils.c $(LOGGER_DIR)/logger.c $(LOGGER_WEB_SRC) $(PORTABLE_DIR)/portable.c $(HOUSE_DIR)/rec.c
TEST_TARGET = test_core
TEST_SRC = tests/test_core.c $(JSON_DIR)/jsonUtils.c $(HOUSE_DIR)/rec.c $(NOTIFICATION_DIR)/notification_lock.c
...
logger-web-test-build:
	$(SSH) $(HOST) 'cd $(DIR) && $(CC) $(LOGGER_WEB_TEST_CFLAGS) $(LOGGER_WEB_TEST_SRC) -o $(LOGGER_WEB_TEST_TARGET) -pthread'
