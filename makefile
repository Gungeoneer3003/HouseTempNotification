...
LOGGER_GRAPH_DIR = $(LOGGER_DIR)/graph
HTTP_DIR = $(SRC_DIR)/http
PORTABLE_DIR = $(SRC_DIR)/portable
RECOMMENDATION_DIR = $(SRC_DIR)/recommendation
SETTINGS_DIR = $(SRC_DIR)/settings
MODULE_DIRS = $(CONFIG_DIR) $(HOUSE_DIR) $(LOCK_DIR) $(JSON_DIR) $(LOGGER_DIR) $(LOGGER_GRAPH_DIR) $(HTTP_DIR) $(PORTABLE_DIR) $(RECOMMENDATION_DIR) $(SETTINGS_DIR)
SRC = $(SRC_DIR)/houseNotif.c $(CONFIG_DIR)/config.c $(HOUSE_DIR)/houseApi.c $(HTTP_DIR)/httpClient.c $(LOCK_DIR)/instanceLock.c $(JSON_DIR)/jsonUtils.c $(LOGGER_DIR)/logger.c $(LOGGER_DIR)/loggerWeb.c $(LOGGER_GRAPH_DIR)/loggerWebGraph.c $(PORTABLE_DIR)/portable.c $(RECOMMENDATION_DIR)/rec.c
...
LOGGER_WEB_TEST_SRC = tests/test_logger_web.c $(LOGGER_DIR)/loggerWeb.c $(LOGGER_GRAPH_DIR)/loggerWebGraph.c
CC = gcc
