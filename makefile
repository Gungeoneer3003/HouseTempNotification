...
MODULE_DIRS = $(CONFIG_DIR) $(HOUSE_DIR) $(LOCK_DIR) $(JSON_DIR) $(LOGGER_DIR) $(LOGGER_WEB_DIR) $(HTTP_DIR) $(PORTABLE_DIR) $(RECOMMENDATION_DIR) $(SETTINGS_DIR)
SRC = $(SRC_DIR)/houseNotif.c $(CONFIG_DIR)/config.c $(HOUSE_DIR)/houseApi.c $(HTTP_DIR)/httpClient.c $(LOCK_DIR)/instanceLock.c $(JSON_DIR)/jsonUtils.c $(LOGGER_DIR)/logger.c $(LOGGER_WEB_DIR)/loggerWeb.c $(LOGGER_WEB_DIR)/loggerWebGraph.c $(PORTABLE_DIR)/portable.c $(RECOMMENDATION_DIR)/rec.c
...
LOGGER_WEB_TEST_SRC = tests/test_logger_web.c $(LOGGER_WEB_DIR)/loggerWeb.c $(LOGGER_WEB_DIR)/loggerWebGraph.c
CC = gcc
