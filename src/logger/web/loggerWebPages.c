...
void loggerWebSendIndex(int client_fd, const LoggerWebServer* server, int is_root) {
    sendPageTemplate(client_fd, server, LOGGER_WEB_LOG_TEMPLATE, is_root, LOGGER_WEB_PAGE_LOG);
}

void loggerWebSendGraphs(int client_fd, const LoggerWebServer* server, int is_root) {
    sendPageTemplate(client_fd, server, LOGGER_WEB_GRAPHS_TEMPLATE, is_root, LOGGER_WEB_PAGE_GRAPHS);
}

void loggerWebSendRawLog(int client_fd, const LoggerWebServer* server, int is_root) {
    sendPageTemplate(client_fd, server, LOGGER_WEB_RAW_TEMPLATE, is_root, LOGGER_WEB_PAGE_RAW);
}
